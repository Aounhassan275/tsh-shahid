...
<div class="row mb-2 mb-xl-4">
    ...
    <h3>View User Deposit Request For Balance | {{App\Models\Setting::siteName()}}</h3>
    </div>
</div>
<div class="col-12 ">
    <div class="card">
        <div class="card-header">
            <h5 class="card-title">View Deposit Request For Balance</h5>
        </div>
        <table id="datatables-buttons" class="table table-striped">
            ...
            <tbody>
                @foreach (App\Models\SimpleDeposit::where('status','Pending')->get() as $key => $deposit)
                <tr> 
                    <td>{{$key+1}}</td>
                    ...
                    <td><a href="{{route('admin.simple-deposit.complete-request',$deposit->id)}}"><button class="btn btn-primary">Active</button></a></td>
                    <td><a href="{{route('admin.simple-deposit.rejected-request',$deposit->id)}}"><button class="btn btn-danger">Reject</button></a></td>
                </tr>
                @endforeach
            </tbody>
        </table>
